...
client.on("message", message => {
	...
	let killPrompt = [
		...
		`Your sword slipped from your hand while swinging and hit your brain`
	];
  ...
  **/
	...
	if (badWords.some(w => message.content.includes(w)) && bWord) {
		...
		message.channel.bulkDelete(1);
		// TODO watch for the below code
		let del = message.channel.send(`Watch your language ${sender}`);
		setTimeout(() => {
			message.channel.bulkDelete(del);
		}, 300);
	}
	if (message.author.hasRoles(["ADMIN"])) {
		// ! n?kick command
		if (messsage.content.startsWith(`${prefix}kick`)) {
		}
	}
});
